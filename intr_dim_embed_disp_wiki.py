...
learning_rate = 1e-3
n_head = 16
n_layer = 20  # scaled to 6 layers
dropout = 0.2
lambda_disp = 0.1 # Weight for dispersion loss
tau_disp = 0.1    # Temperature for angular spread

# intrinsic dimension adjustments
num_matrices = 2
percentweights = 0.05  # amount each layer updates of its assigned matrix
...
if os.path.exists(cache_file):
    ...
    # Helper to encode text with the newly trained merges
...
def angular_spread_loss(x, tau=0.1):
    """
    Computes the Angular Spread Loss from "Dispersing Embeddings in Transformer Layers".
    Encourages embeddings to spread out on the unit hypersphere.
    """
    B, T, D = x.shape
    if T <= 1: return torch.tensor(0.0, device=x.device)
    
    # Normalize to unit hypersphere
    x_norm = F.normalize(x, dim=-1)
    
    # Pairwise cosine similarity: (B, T, D) @ (B, D, T) -> (B, T, T)
    sim = torch.bmm(x_norm, x_norm.transpose(1, 2))
    
    # Angular distance: arccos(cossim) / pi -> Range [0, 1]
    # Clamp for numerical stability
    dist = torch.acos(sim.clamp(-1 + 1e-7, 1 - 1e-7)) / math.pi
    
    # We want to minimize log E[exp(-dist/tau)] for all i != j
    # This pushes points to have larger angular distances
    neg_dist_tau = -dist / tau
    
    # Mask out diagonal (self-similarity)
    mask = torch.eye(T, device=x.device).bool()
    neg_dist_tau = neg_dist_tau.masked_fill(mask, -1e9) # Effectively zero in logsumexp
    
    # LogSumExp over the T*(T-1) pairs
    # LSE(x) - log(N) is log(mean(exp(x)))
    num_pairs = T * (T - 1)
    loss = torch.logsumexp(neg_dist_tau.view(B, -1), dim=-1) - math.log(num_pairs)
...
class Transformer(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        # Collect hidden states for dispersion loss
        hiddens = []
        ...
        if targets is None:
            ...
            # Calculate dispersion loss for each layer's output
            disp_losses = [angular_spread_loss(h, tau=tau_disp) for h in hiddens]
            disp_loss = torch.stack(disp_losses).mean()
            
            # Total loss = CrossEntropy + lambda * Dispersion
            loss = ce_loss + lambda_disp * disp_loss
        return logits, loss
